...
pophist<-ggplot(data)+aes(data=population)+geom_histogram(stat="identity",binwidth=100000)+theme(axis.text.x=element_text(angle = 90, hjust = 1))

#creating histograms of other remaining 3 variables
Assaulthist<-ggplot(data)+aes(data=Assault)+geom_histogram(stat="identity")+theme(axis.text.x=element_text(angle = 90, hjust = 1))
Rapehist<-ggplot(data)+aes(data=Rape)+geom_histogram(stat="identity")+theme(axis.text.x=element_text(angle = 90, hjust = 1))
Urbanpophist<-ggplot(data)+aes(data=Urbanpop)+geom_histogram(stat="identity")+theme(axis.text.x=element_text(angle = 90, hjust = 1))
