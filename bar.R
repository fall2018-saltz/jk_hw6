...
#sorting in terms of Murder rate 
x<-transform(data,stateName=reorder(stateName,Murder,decreasing=T))
...
barmurder3<-ggplot(x)+aes(x=stateName,y=murdersPerState,color=percentOver18)+geom_col()+theme(axis.text.x=element_text(angle = 90, hjust = 1))
